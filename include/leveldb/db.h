...
namespace leveldb {
    ...
    // 数据库特定状态的抽象句柄
    // 快照是一个不变的对象，因此可以从多个线程安全地访问它，而无需任何外部同步。
    class LEVELDB_EXPORT Snapshot { // 快照
    protected:
        virtual ~Snapshot();
    };
     ...
     * 数据库是从键到值的持久有序映射。
     *
     * 数据库对于从多个线程进行并发访问是安全的，而无需任何外部同步。
     */
    class LEVELDB_EXPORT DB {
    public:
        /**
         * 用指定的 "名称" 打开数据库。
         * 将指向堆分配的数据库的指针存储在 *dbptr 中，并在成功时返回 OK。
         * 将 nullptr 存储在 *dbptr 中，并在出错时返回 非OK 状态。
         * 当不再需要 *dbptr 时，呼叫者应将其删除。
         *
         * - Open: 用来打开一个数据
         *
         * DB 的核心操作函数：
