...
namespace leveldb {
    ...
    // 新闻
    // 免费：5个免费源
    // 收费：1月5元、1年45元（20W 约 4545 个年费用户）
    // 收费：1月3元、1年33元（20W 约 6060 个年费用户）
    //
    // 写真 - 不稳定
    // 免费：1天10次
    // 收费：1天2元、1月15元、1年150元（1333 个用户）
    // 野生：1天4元、1月30元、1年300元（666  个用户）
    //
    // 我能做什么？
    // 我的技能：Android、前端
    // 我的目标：提高我自己的资金流，或者直接脱离工作
    // 我可以接外包、做自己的APP、写技术文章、找更好的工作。
    //
    // 接外包累，工资未必还高
    // 做自己的APP，简单易维护，但是量不容易起来，我可以考虑试试收费
...
// encoded using EncodeFixed64.
    ...
    public:
        // 使用 *base 中的文件和 *vset 中的其他信息初始化构建器
    ...
    Status VersionSet::Recover(bool *save_manifest) {
        ...
        std::string current; // 读取 CURRENT 文件中的内容，并保存到 current 变量中。
        ...
        std::string dscname = dbname_ + "/" + current; // 将路径和文件名拼全
        // 在这个路径创建 current 这个文件
        SequentialFile *file;
        s = env_->NewSequentialFile(dscname, &file);
        if (!s.ok()) {
            if (s.IsNotFound()) { // 文件不存在
    ...
    void VersionSet::AddLiveFiles(std::set<uint64_t> *live) {
        // 从当前的 version 中取出下一个 version，判断这两个 version 是否相同，如果不相同，继续循环
